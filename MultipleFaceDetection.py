...
def capture_faces(person_name):
    ...
    while count < 300:  # Changed from 100 to 300
        ret, frame = cap.read()
        if not ret:
            break
        ...
        for (x, y, w, h) in faces:
            ...
            if count >= 300:  # Stop when 300 images are saved
                break
    ...
    print(f"Saved 300 images for {person_name}")
...
def augment_image(img):
    ...
    if random.choice([True, False]):
        img = cv2.GaussianBlur(img, (3, 3), 0)  # Add slight blur
    return img
...
def recognize_faces():
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            threshold = 60  # Adjust threshold for better accuracy
